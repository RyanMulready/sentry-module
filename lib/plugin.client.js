...
import { <%= Object.keys(options.integrations).filter(key => !integrations[key].defaultIntegration).join(',') %> } from '@sentry/integrations'
<% } %>
<% if (options.tracing) { %>
...
export default function (ctx, inject) {
  ...
  config.integrations = [
    <%= Object.entries(options.integrations).map(([name, integration]) => {
      ...
      if(integrationOptions.defaultIntegration) {
        return `new Sentry.${name}({${integrationOptions.join(',')}})`
      } else {
